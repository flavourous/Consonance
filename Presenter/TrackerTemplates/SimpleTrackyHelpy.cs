		...
		{			
			...
			foreach (var target in GetTargets(entry)) {
				...
				if (target.DayPattern.Length == 1)
					kl.Add (TimeSpan.FromDays (target.DayPattern [0]).WithPrefix ().Cap() + " " + target.TargetID, target.DayTargets [0]);
				else
					for (int i = 0; i < target.DayPattern.Length; i++)
						kl.Add (TimeSpan.FromDays (target.DayPattern [i]).WithPrefix (true).Cap() + " " +target.TargetID, target.DayTargets [i]);	
			}
		...
		public IEnumerable<TrackingInfoVM> DetermineInTrackingForDay(Inst di, EntryRetriever<In> eats, EntryRetriever<Out> burns, DateTime dayStart)
		{
			...
			for (var ti=0; ti < targets.Length; ti++) {
                ...
                else
                {
                    ...
                    yret.targetValueName = TimeSpan.FromDays(dtr * fac + fac - 1).WithPrefix().Cap() + " " + targets[ti].TargetName;
                }
